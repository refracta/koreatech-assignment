...
MutexQueue finished_bursts;  // 완료된 burst의 큐
...
Burst *dequeue_valid_burst(MutexQueue *mutex_queue, double current_time);
    ...
    while (true) {
        ...
        if (!is_queue_empty(&mutex_queue->queue)) {
            ...
            if (strcmp(alg, "RR") == 0) {  // 라운드 로빈 알고리즘
                burst = dequeue_valid_burst(mutex_queue, current_time);
            ...
            if (strcmp(alg, "RR") == 0) {  // 라운드 로빈 스케줄링
                ...
                *copy_burst = *burst;
                ...
                if (copy_burst->remaining_time > Q) {
                    ...
                    enqueue(mutex_queue, burst);  // burst를 다시 큐에 추가
                } else {
                    // 타임 퀀텀 이내에 완료될 경우
...
void initialize_simulation() {
    ...
    // 완료된 프로세스를 위한 뮤텍스 초기화
    pthread_mutex_init(&completed_processes_lock, NULL);
    finished_bursts.queue.head = NULL;  // 완료된 burst 큐의 head 초기화
    finished_bursts.queue.tail = NULL;  // 완료된 burst 큐의 tail 초기화
}
...
Burst *dequeue_valid_burst(MutexQueue *mutex_queue, double current_time) {
    ...
    for (Burst *burst = mutex_queue->queue.head; burst != NULL; burst = burst->next, count++) {
        if (burst->arrival_time <= current_time) {
            // 도착 시간이 현재 시간보다 이전인 경우, 해당 burst를 제거
            for (int i = 0; i < count; i++) {
                enqueue(mutex_queue, dequeue(mutex_queue));
            }
            return dequeue(mutex_queue);
        }
    }
...
void output_simulation_results() {
    ...
    int burst_count = 0;
    for (Burst *burst = finished_bursts.queue.head; burst != NULL; burst = burst->next) {
        if (burst->finished == 'Y') {
