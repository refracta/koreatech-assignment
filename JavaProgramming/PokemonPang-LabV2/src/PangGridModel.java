...
//		debugAssign();
    ...
    private void randomAssign() {
        ...
        while (!(pangSequences = PangUtility.getPangSequences(gridData)).isEmpty()) {
            // 검색된 팡 배열이 있으면
            for (PangSequence ps : pangSequences) {
                for (Index2D i : ps.getIndex2DList()) {
                    gridData[i.r()][i.c()] = Pokemon.getRandomNormalPokemonWithIgnoreArray(new Pokemon[]{ps.getType()});
    ...
    public List<PangHintSequence> findHints() {
        return PangUtility.getPangHintSequences(gridData);
    }
