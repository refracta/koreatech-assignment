 ...
 * 과제 1 소스 코드 - EEPROM 멜로디 저장 기능 추가
 */
#include <EEPROM.h>
...
// 버튼 트리거시 재생되는 간이 음악 문자열 설정 (Indiana Jones)
String currentMusic = "5S250eS500pfgp6S63c5S333pS250dS500peS63fS167pS250gS500pabp6S63f5S250paS500pb6S125cde5S250eS500pfgp6S63c5S250p6d5S500p6eS42f5S250gS500pg6S167e5S500p6S250d5S500pg6S167e5S500p6S250d5S500pg6S167f5S500p6S250e5S500p6dS125c";

// EEPROM으로부터 저장한 Music String을 읽어온다.
void writeMusicString(String &musicString)
{
  int len = musicString.length();
  EEPROM.write(0, len & 0xFF);
  EEPROM.write(1, (len & 0xFF00) >> 8);
  // 맨 앞의 두 바이트는 문자열의 길이를 저장하는데 사용한다. 문자열의 길이는 시프트 연산을 통해 바이트로 저장 처리된다.
  for (int i = 0; i < len; i++)
  {
    EEPROM.write(2 + i, musicString[i]);
  }
}

// EEPROM으로부터 저장한 음악 문자열을 읽어온다.
String readMusicString()
{
  int len = (EEPROM.read(1) << 8) + EEPROM.read(0);
  int maxLen = EEPROM.length() - 2;
...
// 저장된 음악 문자열을 확인하고 불러온다.
void setupMusicString() {
  String musicString = readMusicString();
  if(!String("MUSIC:init").equals(musicString) && musicString.startsWith(String("MUSIC:"))) {
    currentMusic = musicString.substring(6);
    Serial.print(F("Music loaded from EEPROM"));
    Serial.print(F(" (Length: "));
    Serial.print(currentMusic.length());
    Serial.print(")");
    Serial.print(F(": "));
    Serial.println(currentMusic);
  } else {
    Serial.println(F("Default music loaded."));
  }
}
...
void handleSerialRequest(){
  if(Serial.available()){
    ...
    currentMusic = Serial.readStringUntil('\n');
    writeMusicString(String("MUSIC:") + currentMusic);

    Serial.print(F("Received successfully."));
    Serial.print(F(" (Length: "));
    Serial.print(currentMusic.length());
    Serial.println(")");
  }
