...
#define INFINITY_MODE true
// AUTO_PLAY_MODE에서만 INFINITY_MODE가 작동함

// value의 비트를 90도 회전시킨다.
unsigned long long rotate(unsigned long long value) {
    unsigned long long result = 0;
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            int flatIndex = i * 8 + j;
            int rotatedIndex = (8 - j - 1) * 8 + i;
...
#define DMImage struct RGDotMatrixImage

struct RGDotMatrixImage {
    unsigned long long redImage;
    unsigned long long greenImage;

    RGDotMatrixImage rotate() {
        return {::rotate(redImage), ::rotate(greenImage)};
    }
    ...
    if (switchCount != 0 && index != lastIndex) {
        ...
        for (int i = 1; i < 8; i++) {
            ...
            while (millis() - time < ANIMATION_DELAY) {
                DMImage previousCopy = previous.rotate();
                previousCopy = (previousCopy >> (i * 8));
                DMImage currentCopy = current.rotate();
                currentCopy = (currentCopy << ((8 - i) * 8));
                DMImage newImage = previousCopy | currentCopy;
                // 90도 회전 후
                // 8칸씩 Shifting 하여 상하로 이동된 이전, 현재 이미지를 만들고 OR(|) 연산하여 합성된 전환 애니메이션 이미지 상을 얻는다.
                newImage = newImage.rotate();
                newImage = newImage.rotate();
                newImage = newImage.rotate();
                // 270도 회전시켜서 회전하기 전의 원래의 상을 얻는다
                displayDMImage(newImage, 0);
            }
        }
        lastIndex = index;
    } else {
