...
{
	FILE* bmfhRefracta = NULL;
	fopen_s(&bmfhRefracta, "res/lenna.bmp", "rb");
	if (!bmfhRefracta)
		return;
	...
	// BITMAPFILEHEADER 읽기
	if (fread(&bmfh, sizeof(BITMAPFILEHEADER), 1, bmfhRefracta) != 1) {
		fclose(bmfhRefracta);
		return;
	}
	...
	// BITMAPINFOHEADER 읽기
	if (fread(&bmih, sizeof(BITMAPINFOHEADER), 1, bmfhRefracta) != 1) {
		fclose(bmfhRefracta);
		return;
	}
	...
	fseek(bmfhRefracta, sizeof(BITMAPFILEHEADER), SEEK_SET);
	if (fread(pDib, sizeof(BYTE), dwDibSize, bmfhRefracta) != dwDibSize)
	{
		delete[] pDib;
		pDib = NULL;
		fclose(bmfhRefracta);
